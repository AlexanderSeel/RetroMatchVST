...
The header also includes a global **ANALOG** character control. It is a continuous 0–100% sound-design macro applied after the complete synth rack: 0% preserves the clean digital signal, while increasing it adds bounded asymmetric saturation, analogue-style slew/bandwidth shaping and warm harmonic density to the whole synthesized instrument. It is persisted in sessions and user presets; factory presets carry authored starting values so the bank demonstrates both clean digital and warm vintage directions.

Beside the global controls, **PANIC** releases synth, sequencer, melody and editor notes on the next audio block, while the adjacent **CPU** readout shows smoothed audio-thread load for the current block size and complete effect/rack path.

### Sequencer / arpeggiator

The editor has a dedicated **SEQUENCER** tab for note-gated sound design as well as melodic phrases. Its graphical step surface shows pitch, velocity/gate and two timbre lanes directly. It supports 64 steps, pattern and held-note arpeggiator modes, internal or DAW-synced clocking, swing, ratchets, ties, note probability, modulation probability and per-step micro-timing. Two macro lanes can target cutoff, resonance, pitch, amplitude, wavetable position/warp, FM amount, wavefold, envelope attack/decay/release, stereo width and reverb/delay/chorus sends using Hold, Linear, Smooth or bounded Random interpolation with independent lane rates. **MOTION ONLY** advances those lanes without emitting sequencer notes, so a held pad, bass, drone or riser can develop a complex timbre without behaving like a second MIDI player. The motion target can be Global, Main Instance or a selected Layer Instance; independent clocks/phrases per instance remain a later DSP extension.

Sequencer state is included in `.rmsynth` plug-in state. The tab includes twelve reusable musical starting patterns, and the **PATTERN MENU** groups them into Bass/Pulse, Arp/Melody, Gate/Groove and Atmosphere/Texture submenus. The Sequence factory presets rotate through ten authored phrase families—bass ostinatos, chord pulses, syncopation, risers, grooves, probabilistic textures and multiple arpeggiator directions—so adjacent presets do not repeat the same motif. Selected Bass, Pad, Texture, Drone, Lead, Pluck and Layered Pad presets also use note-gated MOTION ONLY lanes to deepen a held sound without adding a second melody.
