...
{
    {
        ...
        if (! telemetry.isFinite() || telemetry.totalSamples() != 16
            ...
            return fail ("silence telemetry is not neutral and finite");
    }

    {
        ...
        if (! telemetry.isFinite() || telemetry.finiteSamples != 4 || telemetry.nonFiniteSamples != 0
            ...
            return fail ("known finite fixture produced incorrect peak/RMS/DC/crest/clipping telemetry");

        const auto relaxed = RenderTelemetry::analyze (audio, 1.5f);
        if (relaxed.clippedSamples != 0 || relaxed.hasHardClipping())
            return fail ("custom clipping threshold was ignored");
    }

    {
        ...
        if (telemetry.isFinite() || telemetry.totalSamples() != 4
            ...
            return fail ("non-finite samples were not isolated from finite telemetry");
    }
